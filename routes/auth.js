...
router.post('/register', async (req, res) => {
  try {
    ...
    if (db.dialect === 'postgres') {
      const result = await db.raw.query('INSERT INTO users (username,password,name,email,role) VALUES ($1,$2,$3,$4,$5) RETURNING id',[username,hashedPassword,name,email,role||'tech']);
      return res.status(201).json({ success:true, id:result.rows[0].id });
    }
    db.run('INSERT INTO users (username,password,name,email,role) VALUES (?,?,?,?,?)',[username,hashedPassword,name,email,role||'tech'],function(err){
