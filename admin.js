...
(function(){
  ...
  async function workbookR(d){
    ...
    var ws=wb.addWorksheet('Teknik Servis Raporu',{views:[{state:'frozen',ySplit:7}]});ws.pageSetup={orientation:'landscape',fitToPage:true,fitToWidth:1,fitToHeight:1,paperSize:9};
    ...
    ws.mergeCells('A2:F2');ws.getCell('A2').value='Dönem: '+trR(d.start)+' - '+trR(d.end);ws.getCell('A2').font={bold:true,color:{argb:'FF334155'}};ws.getCell('A2').alignment={horizontal:'center'};
    ...
    var head=['Firma','Teknisyen (Kod)','Ziyaret Tarihi','Durum','Firma Kayıtlı Periyodu','Plan Durumu / Kurulum'];ws.addRow([]);ws.addRow(head);var hr=ws.lastRow;hr.font={bold:true,color:{argb:'FFFFFFFF'}};hr.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF0B3A78'}};hr.alignment={horizontal:'center'};
    d.rows.forEach(function(r){var row=ws.addRow([r.firma,r.teknisyen+' ('+r.techCode+')',r.tarih,r.durum,r.periyot,r.plan+(r.setupInfo||'')]);row.getCell(6).fill={type:'pattern',pattern:'solid',fgColor:{argb:r.plan==='Plana Uygun'?'FFC6EFCE':'FFFCE4D6'}};});
