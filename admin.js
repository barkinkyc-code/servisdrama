...
(function(){
  ...
  function dashboardR(d){
    ...
    var rows=d.rows.map(function(r){var planCls=r.plan==='Plana Uygun'?'ok':(r.plan==='Program Dışı'?'off':'warn');return'<tr><td>'+escR(r.firma)+'</td><td>'+escR(r.teknisyen)+'</td><td>'+r.tarih+'</td><td><span class="wr-badge ok">'+r.durum+'</span></td><td>'+escR(r.lastVisit||'—')+'</td><td><span class="wr-score '+(r.score>=80?'good':r.score>=60?'watch':'risk')+'"><strong>'+(r.score==null?'—':r.score)+'</strong><small>'+(r.score==null?'':escR(r.scoreLabel))+'</small></span></td><td><span class="wr-badge '+planCls+'">'+r.plan+'</span></td><td>'+escR(r.not||'')+'</td></tr>';}).join('');
  ...
  async function workbookR(d){
    ...
    ws.properties.defaultRowHeight=20;ws.pageSetup={orientation:'landscape',paperSize:9,fitToPage:true,fitToWidth:1,fitToHeight:0,margins:{left:0.25,right:0.25,top:0.45,bottom:0.45,header:0.15,footer:0.15},horizontalCentered:true};ws.headerFooter.oddFooter='&LServisDrama&CPage &P / &N&RPowered by BKAYACI';
    ...
    d.rows.forEach(function(r,i){var row=ws.addRow([r.firma,r.teknisyen+' ('+r.techCode+')',r.tarih,r.durum,r.lastVisit||'—',r.score==null?'—':r.score+' / 100\n'+r.scoreLabel,r.plan+(r.setupInfo||''),r.not||'']);row.height=23;var zebra=i%2?'FFF7FAFC':'FFFFFFFF';row.eachCell({includeEmpty:true},function(c){c.fill={type:'pattern',pattern:'solid',fgColor:{argb:zebra}};c.font={size:9,color:{argb:TEXT}};c.alignment={vertical:'middle',wrapText:true};c.border={bottom:{style:'hair',color:{argb:BORDER}}};});row.getCell(3).alignment={horizontal:'center',vertical:'middle'};row.getCell(4).font={bold:true,color:{argb:GREEN}};row.getCell(4).alignment={horizontal:'center',vertical:'middle'};row.getCell(5).alignment={horizontal:'center',vertical:'middle'};var sc=row.getCell(6),scColor=r.score>=80?GREEN:(r.score>=60?ORANGE:'FFDC2626'),scFill=r.score>=80?'FFEAF7EF':(r.score>=60?'FFFFF4E5':'FFFDECEC');sc.font={bold:true,size:9,color:{argb:scColor}};sc.fill={type:'pattern',pattern:'solid',fgColor:{argb:scFill}};sc.alignment={horizontal:'center',vertical:'middle',wrapText:true};sc.border={top:{style:'thin',color:{argb:scColor}},left:{style:'thin',color:{argb:scColor}},bottom:{style:'thin',color:{argb:scColor}},right:{style:'thin',color:{argb:scColor}}};var pc=row.getCell(7);pc.fill={type:'pattern',pattern:'solid',fgColor:{argb:planColors[r.plan]||'FF64748B'}};pc.font={bold:true,size:9,color:{argb:'FFFFFFFF'}};pc.alignment={horizontal:'center',vertical:'middle',wrapText:true};});
