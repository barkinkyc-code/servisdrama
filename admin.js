...
(function(){
  ...
  function reportDataR(start,end){
    ...
    (SD.extras||[]).forEach(function(ex){
      ...
      rows.push({firma:co?co.name:(ex.firmAdi||'Program Dışı Firma'),teknisyen:exTech.name||ex.techCode||'—',tarih:trR(dt),date:dt,durum:'Tamamlandı',periyot:co?periodTextR(weeks):'Kayıtlı periyot yok',plan:'Plan Dışı',firmaId:co?co.id:'extra',techCode:ex.techCode||exTech.code||'—',not:ex.not||''});
    });
  ...
  function dashboardR(d){
    ...
    var rows=d.rows.map(function(r){return'<tr><td>'+escR(r.firma)+'</td><td>'+escR(r.teknisyen)+'</td><td>'+r.tarih+'</td><td><span class="wr-badge ok">'+r.durum+'</span></td><td>'+escR(r.periyot)+'</td><td><span class="wr-badge '+(r.plan==='Plana Uygun'?'ok':'warn')+'">'+r.plan+'</span></td><td>'+escR(r.not||'')+'</td></tr>';}).join('');
  ...
  async function workbookR(d){
    ...
    ws.mergeCells('A1:G1');ws.getCell('A1').value='TEKNİK SERVİS HAFTALIK RAPORU';ws.getCell('A1').font={size:22,bold:true,color:{argb:'FFFFFFFF'}};ws.getCell('A1').fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF0B3A78'}};ws.getCell('A1').alignment={vertical:'middle',horizontal:'center'};ws.getRow(1).height=34;
    ws.mergeCells('A2:G2');ws.getCell('A2').value='Dönem: '+trR(d.start)+' - '+trR(d.end);ws.getCell('A2').font={bold:true,color:{argb:'FF334155'}};ws.getCell('A2').alignment={horizontal:'center'};
    ...
    d.rows.forEach(function(r){var row=ws.addRow([r.firma,r.teknisyen+' ('+r.techCode+')',r.tarih,r.durum,r.periyot,r.plan+(r.setupInfo||''),r.not||'']);row.getCell(6).fill={type:'pattern',pattern:'solid',fgColor:{argb:r.plan==='Plana Uygun'?'FFC6EFCE':'FFFCE4D6'}};});
