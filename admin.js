...
(function(){
  ...
  function reportCompanyR(id){return (SD.companies||[]).find(function(c){return String(c.id)===String(id);})||null;}
  function allCompanyVisitsR(companyId){
    var out=[];
    Object.keys(SD.visits||{}).forEach(function(key){
      if(key.indexOf(String(companyId)+'_')!==0)return;
      var rec=(SD.visits||{})[key];if(!rec||rec.status!=='done')return;
      var dt=null;if(rec.dateISO)dt=new Date(rec.dateISO+'T12:00:00');
      if(!dt||isNaN(dt.getTime())){var m=String(rec.date||'').match(/(\d{1,2})[.\/-](\d{1,2})(?:[.\/-](\d{4}))?/);if(m)dt=new Date(Number(m[3]||new Date().getFullYear()),Number(m[2])-1,Number(m[1]));}
      if(dt&&!isNaN(dt.getTime()))out.push({date:dt,record:rec});
    });
    (SD.extras||[]).forEach(function(ex){if(String(ex.firmaId)!==String(companyId))return;var m=String(ex.tarih||ex.date||'').match(/(\d{1,2})[.\/-](\d{1,2})(?:[.\/-](\d{4}))?/);if(m){var dt=new Date(Number(m[3]||new Date().getFullYear()),Number(m[2])-1,Number(m[1]));out.push({date:dt,record:ex});}});
    return out.sort(function(a,b){return b.date-a.date;});
  }
  function companyHealthR(company){
    if(!company)return{score:null,grade:'-',label:'Kayıtsız'};
    var visits=allCompanyVisitsR(company.id),score=100,now=new Date(),last=visits[0]?visits[0].date:null;
    if(!last)score-=25;else{var days=Math.floor((now-last)/86400000);if(days>120)score-=35;else if(days>60)score-=20;else if(days>30)score-=8;}
    var open=(SD.samples||SD.numuneler||[]).filter(function(x){return String(x.firmaId||x.companyId)===String(company.id)&&!/(tamam|kapalı|sonuçlandı)/i.test(String(x.status||x.durum||''));}).length;
    score-=Math.min(25,open*8);score=Math.max(0,Math.min(100,score));
    var grade=score>=90?'A+':score>=80?'A':score>=70?'B+':score>=60?'B':score>=40?'C':'D';
    return{score:score,grade:grade,label:score>=80?'Sağlıklı':score>=60?'Takip':score>=40?'Riskli':'Kritik'};
  }
  function previousVisitR(row){var co=reportCompanyR(row.firmaId);if(!co)return null;var current=row.date?new Date(row.date):null;var visits=allCompanyVisitsR(co.id).filter(function(v){return !current||v.date<current;});return visits.length?visits[0].date:null;}
  function colorForScoreR(score){return score>=80?'#16A34A':score>=60?'#F59E0B':score>=40?'#F97316':'#DC2626';}
  function pctR(a,b){return b?Math.round(a/b*100):0;}
  async function imageDataR(url){try{var res=await fetch(url);if(!res.ok)return null;var blob=await res.blob();return await new Promise(function(resolve){var fr=new FileReader();fr.onload=function(){resolve(fr.result);};fr.onerror=function(){resolve(null);};fr.readAsDataURL(blob);});}catch(e){return null;}}
  function chartBarsR(obj,maxWidth){
    var keys=Object.keys(obj),max=Math.max.apply(null,keys.map(function(k){return obj[k];}).concat([1]));
    var body=[];keys.forEach(function(k){body.push({columns:[{text:k,width:85,fontSize:8,color:'#16355F'},{canvas:[{type:'rect',x:0,y:2,w:maxWidth,h:8,r:2,color:'#E8EEF6'},{type:'rect',x:0,y:2,w:Math.max(8,maxWidth*obj[k]/max),h:8,r:2,color:'#2367B1'}],width:maxWidth},{text:String(obj[k]),width:20,bold:true,fontSize:9,alignment:'right',color:'#0F2D55'}],margin:[0,4,0,4]});});
    return body.length?body:[{text:'Kayıt yok',fontSize:9,color:'#64748B'}];
  }
  async function pdfDefinitionR(d){
    if(typeof pdfMake==='undefined')throw new Error('PDF kütüphanesi yüklenemedi. İnternet bağlantısını kontrol edin.');
    var logo=await imageDataR('assets/email/servisdrama/drama-makine-logo.png');
    var scored=d.rows.map(function(r){var co=reportCompanyR(r.firmaId),h=companyHealthR(co),prev=previousVisitR(r);return Object.assign({},r,{health:h,previous:prev});});
    var validScores=scored.filter(function(r){return r.health.score!==null;});
    var avg=validScores.length?Math.round(validScores.reduce(function(t,r){return t+r.health.score;},0)/validScores.length*10)/10:0;
    var compliance=pctR(d.uygun,d.total);
    var gradeCounts={'A+':0,'A':0,'B+':0,'B':0,'C':0,'D':0};validScores.forEach(function(r){gradeCounts[r.health.grade]=(gradeCounts[r.health.grade]||0)+1;});
    function kpi(label,value,sub,color){return{width:'*',table:{widths:['*'],body:[[{margin:[10,8,10,8],stack:[{text:label,fontSize:7,bold:true,color:'#17355F'},{text:String(value),fontSize:19,bold:true,color:color,margin:[0,3,0,2]},{text:sub||' ',fontSize:6.5,color:'#64748B'}]}]]},layout:{fillColor:'#FFFFFF',hLineColor:'#DCE4EE',vLineColor:'#DCE4EE',hLineWidth:function(){return .6;},vLineWidth:function(){return .6;}}};}
    var summaryRows=scored.slice(0,8).map(function(r){return[r.firma,r.teknisyen,r.tarih,{text:r.durum,color:'#168A4B',bold:true},r.previous?trR(r.previous):'İlk ziyaret',{text:r.health.score===null?'-':r.health.grade+'  '+r.health.score,color:r.health.score===null?'#64748B':colorForScoreR(r.health.score),bold:true,alignment:'center'},{text:r.plan,color:r.plan==='Plana Uygun'?'#168A4B':r.plan==='Program Dışı'?'#6D3FD1':'#E67800',bold:true},r.not||'-'];});
    var detailRows=scored.map(function(r){return[r.firma,r.teknisyen,r.tarih,r.previous?trR(r.previous):'İlk ziyaret',r.health.score===null?'-':String(r.health.score),r.health.grade,r.plan,r.not||'-'];});
    var doc={
      pageSize:'A4',pageOrientation:'landscape',pageMargins:[22,22,22,28],
      defaultStyle:{font:'Roboto',fontSize:8,color:'#102B50'},
      footer:function(current,count){return{margin:[22,6,22,0],columns:[{text:'ServisDrama • Haftalık Rapor',fontSize:7,color:'#6B7B90'},{text:'Powered by BKAYACI',alignment:'center',fontSize:7,color:'#6B7B90'},{text:'Sayfa '+current+' / '+count,alignment:'right',fontSize:7,color:'#6B7B90'}]};},
      content:[
        {columns:[logo?{image:logo,width:104,margin:[0,0,16,0]}:{text:'DRAMA MAKİNE',bold:true,fontSize:14,width:120},{stack:[{text:'TEKNİK SERVİS HAFTALIK RAPORU',fontSize:20,bold:true,color:'#0B2F67'},{text:'SERVİSDRAMA',fontSize:8,bold:true,color:'#2875C7',margin:[0,2,0,0]}]},{width:205,columns:[{stack:[{text:'DÖNEM',fontSize:7,bold:true},{text:trR(d.start)+' - '+trR(d.end),fontSize:9,bold:true}],margin:[10,0,0,0]},{stack:[{text:'RAPOR TARİHİ',fontSize:7,bold:true},{text:trR(new Date()),fontSize:9,bold:true}],margin:[10,0,0,0]}]}],columnGap:8},
        {canvas:[{type:'line',x1:0,y1:5,x2:797,y2:5,lineWidth:2,lineColor:'#0B3A78'}],margin:[0,3,0,10]},
        {columns:[kpi('TOPLAM ZİYARET',d.total,'Haftalık gerçekleşen','#1765B6'),kpi('ZİYARET EDİLEN FİRMA',d.unique,'Benzersiz firma','#168A4B'),kpi('PLANA UYGUN',d.uygun,'Plan içi ziyaret','#16A34A'),kpi('PLAN DIŞI',d.planDisi,'Takvim dışı','#E67800'),kpi('PROGRAM DIŞI',d.programDisi,'Ek saha işlemi','#6D3FD1'),kpi('ORTALAMA FİRMA SKORU',avg,validScores.length+' firma','#0B3A78'),kpi('PLAN UYUM ORANI','%'+compliance,'Plana uygun / toplam','#1765B6')],columnGap:7,margin:[0,0,0,12]},
        {columns:[
          {width:'25%',table:{widths:['*'],body:[[{stack:[{text:'TEKNİSYENE GÖRE ZİYARET',bold:true,fontSize:10,alignment:'center',margin:[0,0,0,7]}].concat(chartBarsR(d.tech,105)),margin:[8,8,8,8]}]]},layout:'lightHorizontalLines'},
          {width:'25%',table:{widths:['*'],body:[[{stack:[{text:'GÜNLERE GÖRE DAĞILIM',bold:true,fontSize:10,alignment:'center',margin:[0,0,0,7]}].concat(chartBarsR(d.days,105)),margin:[8,8,8,8]}]]},layout:'lightHorizontalLines'},
          {width:'23%',table:{widths:['*'],body:[[{stack:[{text:'PLAN DURUMU DAĞILIMI',bold:true,fontSize:10,alignment:'center',margin:[0,0,0,10]},{columns:[{canvas:[{type:'ellipse',x:42,y:42,r1:36,r2:36,color:'#16A34A'},{type:'ellipse',x:42,y:42,r1:22,r2:22,color:'#FFFFFF'}],width:90},{stack:[{text:'%'+compliance,fontSize:18,bold:true,color:'#16A34A'},{text:'Plana Uygun: '+d.uygun,fontSize:8,margin:[0,4,0,2]},{text:'Program Dışı: '+d.programDisi,fontSize:8,color:'#6D3FD1'},{text:'Plan Dışı: '+d.planDisi,fontSize:8,color:'#E67800',margin:[0,2,0,0]}]}]}],margin:[8,8,8,8]}]]},layout:'lightHorizontalLines'},
          {width:'27%',table:{widths:['*'],body:[[{stack:[{text:'FİRMA SKOR DAĞILIMI',bold:true,fontSize:10,alignment:'center',margin:[0,0,0,8]}].concat(Object.keys(gradeCounts).map(function(g){var ranges={'A+':'90-100','A':'80-89','B+':'70-79','B':'60-69','C':'40-59','D':'0-39'};return{columns:[{text:g,width:26,bold:true,color:g==='A+'||g==='A'?'#168A4B':g==='B+'||g==='B'?'#D28A00':'#DC2626'},{text:'('+ranges[g]+')',width:50,color:'#64748B'},{text:String(gradeCounts[g]),width:18,bold:true,alignment:'right'},{text:'%'+pctR(gradeCounts[g],validScores.length),alignment:'right',color:'#64748B'}],margin:[0,3,0,3]};})),margin:[8,8,8,8]}]]},layout:'lightHorizontalLines'}
        ],columnGap:8,margin:[0,0,0,10]},
        {text:'SON ZİYARETLER - ÖZET',fontSize:10,bold:true,margin:[0,0,0,4]},
        {table:{headerRows:1,widths:[145,82,55,58,64,55,66,'*'],body:[['Firma','Teknisyen (Kod)','Ziyaret Tarihi','Durum','Son Ziyaret','Firma Skoru','Plan Durumu','Not']].concat(summaryRows)},layout:{fillColor:function(i){return i===0?'#0B3A78':i%2===0?'#F6F9FC':'#FFFFFF';},hLineColor:'#DCE4EE',vLineColor:'#DCE4EE',paddingLeft:function(){return 5;},paddingRight:function(){return 5;},paddingTop:function(){return 4;},paddingBottom:function(){return 4;}},fontSize:7.2,color:'#102B50'},
        {text:'Not: Firma skoru son ziyaret güncelliği ve açık numune kayıtları dikkate alınarak hesaplanır.',italics:true,fontSize:6.5,color:'#64748B',margin:[0,5,0,0]},
        {text:'ZİYARET DETAYI',fontSize:12,bold:true,pageBreak:'before',margin:[0,0,0,6]},
        {table:{headerRows:1,widths:[170,90,58,62,42,42,70,'*'],body:[['Firma','Teknisyen (Kod)','Ziyaret Tarihi','Son Ziyaret','Skor','Seviye','Plan Durumu','Not']].concat(detailRows)},layout:{fillColor:function(i){return i===0?'#0B3A78':i%2===0?'#F6F9FC':'#FFFFFF';},hLineColor:'#DCE4EE',vLineColor:'#DCE4EE',paddingLeft:function(){return 5;},paddingRight:function(){return 5;},paddingTop:function(){return 4;},paddingBottom:function(){return 4;}},fontSize:7.2},
        {columns:[{width:'70%',text:'Rapor, seçilen tarih aralığındaki tamamlanmış ziyaretleri içerir. Son Ziyaret alanı, rapordaki ziyaret tarihinden önceki en güncel tamamlanmış kaydı gösterir.',fontSize:7,color:'#64748B',margin:[0,10,10,0]},{width:'30%',table:{widths:['*'],body:[[{stack:[{text:'RAPOR AÇIKLAMALARI',bold:true,fontSize:9,margin:[0,0,0,5]},{text:'Plan Uyum Oranı: Plana uygun ziyaretlerin toplam ziyaretlere oranıdır.',fontSize:7,margin:[0,2,0,2]},{text:'Firma Skoru: Ziyaret güncelliği ve açık numunelere göre hesaplanır.',fontSize:7,margin:[0,2,0,2]},{text:'Son Ziyaret: Mevcut kayıttan önceki son tamamlanmış ziyarettir.',fontSize:7,margin:[0,2,0,2]}],margin:[7,7,7,7]}]]},layout:'lightHorizontalLines'}],margin:[0,0,0,0]}
      ]
    };
    return doc;
  }
  function pdfBase64R(docDef){return new Promise(function(resolve,reject){try{pdfMake.createPdf(docDef).getBase64(function(data){resolve(data);});}catch(e){reject(e);}});}
  window.generateWeeklyReport=async function(){try{var r=rangeR(),d=reportDataR(r.start,r.end);if(!d.total){UI.toast('Seçilen tarih aralığında tamamlanmış ziyaret yok.','warning');return;}UI.toast('Yönetim PDF raporu hazırlanıyor...','info');var def=await pdfDefinitionR(d);pdfMake.createPdf(def).download('Teknik_Servis_Haftalik_Rapor_'+r.start+'_'+r.end+'.pdf');UI.toast('PDF raporu indirildi.','success');}catch(e){console.error(e);UI.toast(e.message||'PDF oluşturulamadı.','error');}};
  window.mailWeeklyReport=async function(){try{var r=rangeR(),d=reportDataR(r.start,r.end);if(!d.total){UI.toast('Mail için rapor kaydı yok.','warning');return;}var cfg=SD.config||{},mc=typeof getMailToCc==='function'?getMailToCc():{to:['barkin.kayaci@dramamakine.com'],cc:[]};UI.toast('PDF eki hazırlanıyor...','info');var def=await pdfDefinitionR(d),b64=await pdfBase64R(def),filename='Teknik_Servis_Haftalik_Rapor_'+r.start+'_'+r.end+'.pdf';var html='<!doctype html><html lang="tr"><body style="margin:0;background:#f3f6fa;font-family:Arial,sans-serif;"><table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:24px"><table width="680" style="background:#fff;border:1px solid #dbe3ec"><tr><td style="padding:20px 30px"><img src="cid:drama-makine-logo" width="158" alt="Drama Makine"></td></tr><tr><td style="padding:30px;background:#102b50;color:#fff"><div style="font-size:12px;letter-spacing:2px;color:#42a1ff;font-weight:bold">SERVİSDRAMA</div><h1 style="margin:8px 0;font-size:30px">Haftalık Teknik Servis Raporu</h1><div>'+trR(d.start)+' - '+trR(d.end)+'</div></td></tr><tr><td style="padding:26px 30px;color:#13233f"><p>Yönetim sunumuna uygun PDF raporu ekte yer almaktadır.</p><table width="100%" style="border-collapse:collapse"><tr><td align="center" style="padding:15px;border:1px solid #dbe3ec"><b style="font-size:22px;color:#1565d8">'+d.total+'</b><br>Toplam Ziyaret</td><td align="center" style="padding:15px;border:1px solid #dbe3ec"><b style="font-size:22px;color:#16a34a">'+d.uygun+'</b><br>Plana Uygun</td><td align="center" style="padding:15px;border:1px solid #dbe3ec"><b style="font-size:22px;color:#6d3fd1">'+d.programDisi+'</b><br>Program Dışı</td></tr></table></td></tr><tr><td align="center" style="padding:18px;color:#7a8799;font-size:12px">ServisDrama • Haftalık Rapor • Powered by BKAYACI</td></tr></table></td></tr></table></body></html>';
    var res=await fetch('/api/send-test-mail',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:mc.to,cc:mc.cc,subject:'Teknik Servis Haftalık Raporu | '+trR(d.start)+' - '+trR(d.end),html:html,smtpHost:cfg.smtpHost,smtpPort:cfg.smtpPort,smtpUser:cfg.smtpUser,smtpPass:cfg.smtpPass,from:'ServisDrama <'+(cfg.smtpSenderEmail||cfg.smtpUser||'kimyaservis@dramamakine.com')+'>',attachmentNames:['drama-makine-logo'],attachments:[{filename:filename,contentBase64:b64,contentType:'application/pdf'}]})});var out=await res.json();if(!res.ok)throw new Error(out.details||out.error||'Mail gönderilemedi');UI.toast('Haftalık rapor PDF ekiyle gönderildi.','success');}catch(e){console.error(e);UI.toast(e.message||'Mail gönderilemedi.','error');}}
