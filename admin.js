...
(function(){
  ...
  function dashboardR(d){
    ...
    var rows=d.rows.map(function(r){var planCls=r.plan==='Plana Uygun'?'ok':(r.plan==='Program Dışı'?'off':'warn');return'<tr><td>'+escR(r.firma)+'</td><td>'+escR(r.teknisyen)+'</td><td>'+r.tarih+'</td><td><span class="wr-badge ok">'+r.durum+'</span></td><td>'+escR(r.lastVisit||'—')+'</td><td><span class="wr-score '+(r.score>=80?'good':r.score>=60?'watch':'risk')+'"><strong>'+scoreGradeR(r.score).grade+'</strong><span class="wr-score-meta"><b>'+(r.score==null?'—':r.score)+'</b><small>'+escR(scoreGradeR(r.score).label)+'</small></span></span></td><td><span class="wr-badge '+planCls+'">'+r.plan+'</span></td><td>'+escR(r.not||'')+'</td></tr>';}).join('');
  ...
  function scoreGradeR(score){
    if(score==null)return{grade:'—',label:'Skor Yok'};
    if(score>=95)return{grade:'A+',label:'Mükemmel'};
    if(score>=85)return{grade:'A',label:'Güçlü'};
    if(score>=75)return{grade:'B+',label:'İyi'};
    if(score>=65)return{grade:'B',label:'Takip'};
    if(score>=50)return{grade:'C',label:'Riskli'};
    return{grade:'D',label:'Kritik'};
  }
  async function workbookR(d){
    ...
    wb.creator='ServisDrama';wb.lastModifiedBy='ServisDrama';wb.created=new Date();wb.modified=new Date();wb.company='Drama Makine';wb.subject='Teknik Servis Haftalık Yönetici Raporu';
    var C={navy:'FF102B50',navy2:'FF173F6D',blue:'FF1565D8',cyan:'FF38A8E8',green:'FF169B62',greenSoft:'FFEAF7F1',orange:'FFF28C28',orangeSoft:'FFFFF3E6',purple:'FF7347C8',purpleSoft:'FFF1ECFB',red:'FFD94B4B',redSoft:'FFFDECEC',ink:'FF17233C',muted:'FF66758A',line:'FFDCE4EE',soft:'FFF5F8FC',white:'FFFFFFFF',gold:'FFC99B35'};
    var thin={style:'thin',color:{argb:C.line}};
    function fill(argb){return{type:'pattern',pattern:'solid',fgColor:{argb:argb}};}
    function borderAll(color){var b={style:'thin',color:{argb:color||C.line}};return{top:b,left:b,bottom:b,right:b};}
    function setBlock(ws,range,bg){ws.getCell(range.split(':')[0]).fill=fill(bg);}
    function addLogo(ws){return fetch('assets/email/servisdrama/drama-makine-logo.png').then(function(resp){if(!resp.ok)return;return resp.arrayBuffer();}).then(function(buf){if(!buf)return;var id=wb.addImage({buffer:buf,extension:'png'});ws.addImage(id,{tl:{col:0.18,row:0.20},ext:{width:142,height:52}});}).catch(function(){});}

    var rate=d.total?Math.round(d.uygun/d.total*100):0;
    var nonPlan=d.planDisi+d.programDisi;
    var grade=scoreGradeR(d.avgScore);
    var ws=wb.addWorksheet('Yönetici Özeti',{views:[{state:'frozen',ySplit:12,showGridLines:false}]});
    ws.properties.defaultRowHeight=20;
    ws.pageSetup={orientation:'landscape',paperSize:9,fitToPage:true,fitToWidth:1,fitToHeight:1,horizontalCentered:true,margins:{left:0.25,right:0.25,top:0.35,bottom:0.35,header:0.12,footer:0.15}};
    ws.headerFooter.oddFooter='&LServisDrama&C&P / &N&RPowered by BKAYACI';
    ws.columns=[{width:22},{width:22},{width:16},{width:16},{width:16},{width:16},{width:16},{width:16},{width:16},{width:16},{width:16},{width:16}];

    ws.mergeCells('A1:C3');ws.mergeCells('D1:I3');ws.mergeCells('J1:L1');ws.mergeCells('J2:L3');
    ['A1','D1','J1','J2'].forEach(function(a){ws.getCell(a).fill=fill(C.navy);});
    ws.getCell('D1').value='TEKNİK SERVİS | HAFTALIK YÖNETİCİ RAPORU';ws.getCell('D1').font={name:'Aptos Display',size:20,bold:true,color:{argb:C.white}};ws.getCell('D1').alignment={horizontal:'center',vertical:'middle'};
    ws.getCell('J1').value='RAPORLAMA DÖNEMİ';ws.getCell('J1').font={name:'Aptos',size:9,bold:true,color:{argb:'FF9FC5EC'}};ws.getCell('J1').alignment={horizontal:'center',vertical:'middle'};
    ws.getCell('J2').value=trR(d.start)+' — '+trR(d.end)+'\nRapor Tarihi: '+trR(new Date());ws.getCell('J2').font={name:'Aptos',size:11,bold:true,color:{argb:C.white}};ws.getCell('J2').alignment={horizontal:'center',vertical:'middle',wrapText:true};
    ws.getRow(1).height=22;ws.getRow(2).height=24;ws.getRow(3).height=24;
    await addLogo(ws);
    ws.mergeCells('A4:L4');ws.getCell('A4').fill=fill(C.cyan);ws.getRow(4).height=5;

    var cards=[
      {r:'A5:B7',label:'TOPLAM ZİYARET',value:d.total,color:C.blue,sub:'Saha faaliyeti'},
      {r:'C5:D7',label:'ZİYARET EDİLEN FİRMA',value:d.unique,color:'FF078578',sub:'Tekil firma'},
      {r:'E5:F7',label:'PLANA UYGUN',value:d.uygun,color:C.green,sub:'Planlı ziyaret'},
      {r:'G5:H7',label:'PLAN DIŞI',value:nonPlan,color:C.orange,sub:d.planDisi+' plan dışı · '+d.programDisi+' program dışı'},
      {r:'I5:J7',label:'PLAN UYUMU',value:'%'+rate,color:rate>=85?C.green:(rate>=70?C.orange:C.red),sub:rate>=85?'Hedef üzerinde':(rate>=70?'İzlenmeli':'Aksiyon gerekli')},
      {r:'K5:L7',label:'PORTFÖY SKORU',value:grade.grade+'  '+d.avgScore,color:d.avgScore>=80?C.green:(d.avgScore>=60?C.orange:C.red),sub:grade.label+' · 100 üzerinden'}
    ];
    cards.forEach(function(k){ws.mergeCells(k.r);var c=ws.getCell(k.r.split(':')[0]);c.value={richText:[{font:{name:'Aptos',size:9,bold:true,color:{argb:C.muted}},text:k.label+'\n'},{font:{name:'Aptos Display',size:22,bold:true,color:{argb:k.color}},text:String(k.value)+'\n'},{font:{name:'Aptos',size:8,color:{argb:C.muted}},text:k.sub}]};c.alignment={horizontal:'center',vertical:'middle',wrapText:true};c.fill=fill(C.white);c.border=borderAll(C.line);});
    [5,6,7].forEach(function(r){ws.getRow(r).height=21;});
    ws.getRow(8).height=8;

    ws.mergeCells('A9:F9');ws.mergeCells('G9:L9');
    ['A9','G9'].forEach(function(a){var c=ws.getCell(a);c.fill=fill(C.navy2);c.font={name:'Aptos',size:10,bold:true,color:{argb:C.white}};c.alignment={vertical:'middle'};});
    ws.getCell('A9').value='  TEKNİSYEN PERFORMANSI';ws.getCell('G9').value='  GÜNLERE GÖRE SAHA YOĞUNLUĞU';ws.getRow(9).height=25;
    var techKeys=Object.keys(d.tech).sort(function(a,b){return d.tech[b]-d.tech[a];});
    var dayKeys=Object.keys(d.days).sort(function(a,b){var pa=a.split('.'),pb=b.split('.');return new Date(pa[2],pa[1]-1,pa[0])-new Date(pb[2],pb[1]-1,pb[0]);});
    var maxTech=Math.max.apply(null,techKeys.map(function(k){return d.tech[k];}).concat([1]));
    var maxDay=Math.max.apply(null,dayKeys.map(function(k){return d.days[k];}).concat([1]));
    var summaryRows=Math.max(techKeys.length,dayKeys.length,3);
    for(var si=0;si<summaryRows;si++){
      var rr=10+si;ws.getRow(rr).height=24;
      ws.mergeCells('A'+rr+':B'+rr);ws.mergeCells('C'+rr+':E'+rr);ws.mergeCells('G'+rr+':H'+rr);ws.mergeCells('I'+rr+':K'+rr);
      var tk=techKeys[si],dk=dayKeys[si];
      ws.getCell('A'+rr).value=tk||'';ws.getCell('A'+rr).font={name:'Aptos',size:9,bold:true,color:{argb:C.ink}};ws.getCell('A'+rr).alignment={vertical:'middle'};
      ws.getCell('C'+rr).value=tk?Array(Math.max(2,Math.round(d.tech[tk]/maxTech*14))).join('●'):'';ws.getCell('C'+rr).font={name:'Aptos',size:9,color:{argb:C.blue}};ws.getCell('C'+rr).alignment={vertical:'middle'};
      ws.getCell('F'+rr).value=tk?d.tech[tk]:'';ws.getCell('F'+rr).font={name:'Aptos Display',size:12,bold:true,color:{argb:C.navy}};ws.getCell('F'+rr).alignment={horizontal:'center',vertical:'middle'};
      ws.getCell('G'+rr).value=dk||'';ws.getCell('G'+rr).font={name:'Aptos',size:9,bold:true,color:{argb:C.ink}};ws.getCell('G'+rr).alignment={vertical:'middle'};
      ws.getCell('I'+rr).value=dk?Array(Math.max(2,Math.round(d.days[dk]/maxDay*14))).join('●'):'';ws.getCell('I'+rr).font={name:'Aptos',size:9,color:{argb:'FF2E77C3'}};ws.getCell('I'+rr).alignment={vertical:'middle'};
      ws.getCell('L'+rr).value=dk?d.days[dk]:'';ws.getCell('L'+rr).font={name:'Aptos Display',size:12,bold:true,color:{argb:C.navy}};ws.getCell('L'+rr).alignment={horizontal:'center',vertical:'middle'};
      for(var cc=1;cc<=12;cc++){ws.getCell(rr,cc).fill=fill(si%2?C.soft:C.white);ws.getCell(rr,cc).border={bottom:thin};}
    }
    var detailStart=10+summaryRows+1;
    ws.mergeCells('A'+detailStart+':L'+detailStart);var sec=ws.getCell('A'+detailStart);sec.value='ZİYARET PORTFÖYÜ — YÖNETİCİ ÖZETİ';sec.fill=fill(C.navy);sec.font={name:'Aptos',size:10,bold:true,color:{argb:C.white}};sec.alignment={vertical:'middle'};ws.getRow(detailStart).height=27;
    var headers=['Firma','Teknisyen','Tarih','Son Ziyaret','Skor','Seviye','Plan Durumu','Not'];
    var hdrRow=detailStart+1;
    var ranges=[['A','D'],['E','F'],['G','G'],['H','I'],['J','J'],['K','K'],['L','L']];
    ws.mergeCells('A'+hdrRow+':D'+hdrRow);ws.mergeCells('E'+hdrRow+':F'+hdrRow);ws.mergeCells('H'+hdrRow+':I'+hdrRow);
    var hs=['A','E','G','H','J','K','L'],hv=['Firma','Teknisyen','Ziyaret Tarihi','Son Ziyaret','Skor','Seviye / Plan','Not'];
    hs.forEach(function(col,i){var c=ws.getCell(col+hdrRow);c.value=hv[i];c.fill=fill(C.navy2);c.font={name:'Aptos',size:9,bold:true,color:{argb:C.white}};c.alignment={horizontal:'center',vertical:'middle',wrapText:true};c.border=borderAll('FF3F5E82');});ws.getRow(hdrRow).height=25;
    var maxPreview=Math.min(d.rows.length,18);
    for(var i=0;i<maxPreview;i++){
      var r=d.rows[i],row=hdrRow+1+i,g=scoreGradeR(r.score),scoreColor=r.score>=80?C.green:(r.score>=60?C.orange:C.red),planColor=r.plan==='Plana Uygun'?C.green:(r.plan==='Program Dışı'?C.purple:C.orange);
      ws.mergeCells('A'+row+':D'+row);ws.mergeCells('E'+row+':F'+row);ws.mergeCells('H'+row+':I'+row);
      ws.getCell('A'+row).value=r.firma;ws.getCell('E'+row).value=r.teknisyen+' ('+r.techCode+')';ws.getCell('G'+row).value=r.tarih;ws.getCell('H'+row).value=r.lastVisit||'—';ws.getCell('J'+row).value=g.grade+'  '+(r.score==null?'—':r.score);ws.getCell('K'+row).value=g.label+' · '+r.plan;ws.getCell('L'+row).value=r.not||'';
      for(var cidx=1;cidx<=12;cidx++){var c=ws.getCell(row,cidx);c.fill=fill(i%2?C.soft:C.white);c.font={name:'Aptos',size:8.5,color:{argb:C.ink}};c.alignment={vertical:'middle',wrapText:true};c.border={bottom:thin};}
      ws.getCell('G'+row).alignment={horizontal:'center',vertical:'middle'};ws.getCell('H'+row).alignment={horizontal:'center',vertical:'middle'};
      ws.getCell('J'+row).font={name:'Aptos Display',size:10,bold:true,color:{argb:scoreColor}};ws.getCell('J'+row).alignment={horizontal:'center',vertical:'middle'};
      ws.getCell('K'+row).font={name:'Aptos',size:8,bold:true,color:{argb:planColor}};ws.getCell('K'+row).alignment={horizontal:'center',vertical:'middle',wrapText:true};ws.getRow(row).height=24;
    }
    var footRow=hdrRow+maxPreview+2;ws.mergeCells('A'+footRow+':L'+footRow);var foot=ws.getCell('A'+footRow);foot.value='ServisDrama  •  Haftalık Yönetici Raporu  •  Detaylı kayıtlar “Ziyaret Detayı” sayfasındadır  •  Powered by BKAYACI';foot.fill=fill(C.soft);foot.font={name:'Aptos',size:8,color:{argb:C.muted}};foot.alignment={horizontal:'center',vertical:'middle'};ws.getRow(footRow).height=22;
    ws.printArea='A1:L'+footRow;

    var dt=wb.addWorksheet('Ziyaret Detayı',{views:[{state:'frozen',ySplit:6,showGridLines:false}]});
    dt.properties.defaultRowHeight=19;dt.pageSetup={orientation:'landscape',paperSize:9,fitToPage:true,fitToWidth:1,fitToHeight:0,margins:{left:0.2,right:0.2,top:0.3,bottom:0.3,header:0.1,footer:0.15}};dt.headerFooter.oddFooter='&LServisDrama&C&P / &N&RPowered by BKAYACI';
    dt.columns=[{width:42},{width:24},{width:15},{width:15},{width:17},{width:12},{width:18},{width:25},{width:32}];
    dt.mergeCells('A1:B3');dt.mergeCells('C1:G3');dt.mergeCells('H1:I1');dt.mergeCells('H2:I3');['A1','C1','H1','H2'].forEach(function(a){dt.getCell(a).fill=fill(C.navy);});
    dt.getCell('C1').value='TEKNİK SERVİS | ZİYARET DETAYI';dt.getCell('C1').font={name:'Aptos Display',size:19,bold:true,color:{argb:C.white}};dt.getCell('C1').alignment={horizontal:'center',vertical:'middle'};
    dt.getCell('H1').value='DÖNEM';dt.getCell('H1').font={name:'Aptos',size:9,bold:true,color:{argb:'FF9FC5EC'}};dt.getCell('H1').alignment={horizontal:'center',vertical:'middle'};
    dt.getCell('H2').value=trR(d.start)+' — '+trR(d.end);dt.getCell('H2').font={name:'Aptos',size:11,bold:true,color:{argb:C.white}};dt.getCell('H2').alignment={horizontal:'center',vertical:'middle'};await addLogo(dt);
    dt.mergeCells('A4:I4');dt.getCell('A4').fill=fill(C.cyan);dt.getRow(4).height=5;dt.getRow(5).height=7;
    var head=['Firma','Teknisyen (Kod)','Ziyaret Tarihi','Durum','Son Ziyaret','Skor','Seviye','Plan Durumu / Kurulum','Not'];dt.addRow(head);var hr=dt.lastRow;hr.height=28;hr.font={name:'Aptos',bold:true,size:9,color:{argb:C.white}};hr.fill=fill(C.navy2);hr.alignment={horizontal:'center',vertical:'middle',wrapText:true};hr.eachCell(function(c){c.border=borderAll('FF3F5E82');});
    d.rows.forEach(function(r,i){var g=scoreGradeR(r.score),row=dt.addRow([r.firma,r.teknisyen+' ('+r.techCode+')',r.tarih,r.durum,r.lastVisit||'—',r.score==null?'—':r.score,g.grade+' · '+g.label,r.plan+(r.setupInfo||''),r.not||'']);row.height=23;var zebra=i%2?C.soft:C.white;row.eachCell({includeEmpty:true},function(c){c.fill=fill(zebra);c.font={name:'Aptos',size:8.5,color:{argb:C.ink}};c.alignment={vertical:'middle',wrapText:true};c.border={bottom:thin};});row.getCell(3).alignment={horizontal:'center',vertical:'middle'};row.getCell(4).font={name:'Aptos',bold:true,color:{argb:C.green}};row.getCell(4).alignment={horizontal:'center',vertical:'middle'};row.getCell(5).alignment={horizontal:'center',vertical:'middle'};
      var scoreColor=r.score>=80?C.green:(r.score>=60?C.orange:C.red);row.getCell(6).font={name:'Aptos Display',size:11,bold:true,color:{argb:scoreColor}};row.getCell(6).alignment={horizontal:'center',vertical:'middle'};row.getCell(7).font={name:'Aptos',size:8,bold:true,color:{argb:scoreColor}};row.getCell(7).alignment={horizontal:'center',vertical:'middle'};
      var planColor=r.plan==='Plana Uygun'?C.green:(r.plan==='Program Dışı'?C.purple:C.orange);row.getCell(8).font={name:'Aptos',size:8.5,bold:true,color:{argb:planColor}};row.getCell(8).alignment={horizontal:'center',vertical:'middle',wrapText:true};
    });
    dt.autoFilter={from:'A6',to:'I6'};var last=6+d.rows.length;dt.mergeCells('A'+(last+2)+':I'+(last+2));dt.getCell('A'+(last+2)).value='ServisDrama  •  Teknik Servis Haftalık Raporu  •  www.dramamakine.com  •  Powered by BKAYACI';dt.getCell('A'+(last+2)).fill=fill(C.soft);dt.getCell('A'+(last+2)).font={name:'Aptos',size:8,color:{argb:C.muted}};dt.getCell('A'+(last+2)).alignment={horizontal:'center',vertical:'middle'};dt.getRow(last+2).height=22;dt.printArea='A1:I'+(last+2);
    return wb;
  }
