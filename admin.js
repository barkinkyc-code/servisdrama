...
(function(){
  ...
  function dashboardR(d){
    ...
    return'<div class="weekly-report">'
      +'<div class="wr-top"><div class="wr-logo">DK<small>PORTAL</small></div><div class="wr-title"><h2>TEKNİK SERVİS HAFTALIK RAPORU</h2><div class="wr-meta"><span>▣ Dönem: <b>'+trR(d.start)+' - '+trR(d.end)+'</b></span><span>▣ Rapor Tarihi: <b>'+trR(new Date())+'</b></span><span>▤ Toplam Kayıt: <b>'+d.total+'</b></span><span>♙ Rapor Sahibi: <b>Barkın Kayacı</b></span></div></div><div class="wr-actions"><button class="primary" onclick="generateWeeklyReport()">Excel İndir</button><button onclick="mailWeeklyReport()">Mail Gönder</button></div></div>'
      +'<div class="wr-kpis">'+kpiR('Toplam Ziyaret',d.total,'visit','blue')+kpiR('Ziyaret Edilen Firma Toplamı',d.unique,'company','navy')+techKeys.map(function(k,i){return kpiR(k,d.tech[k],'user',i%2?'green':'blue');}).join('')+kpiR('Numune Alımı',sampleCount,'sample','purple')+kpiR('Plan Dışı',d.disi,'warn','red')+kpiR('Plana Uygun',d.uygun,'ok','green')+'</div>'
      +'<div class="wr-charts"><section><h3>1) Teknisyene Göre Ziyaret Sayısı</h3>'+barsR(d.tech)+'</section><section><h3>2) Günlere Göre Ziyaret Dağılımı</h3>'+barsR(d.days)+'</section><section><h3>3) Ziyaret / Numune Dağılımı</h3><div class="wr-simple-donut"><div><b>'+d.total+'</b><span>Toplam</span></div></div><div class="wr-legend"><span><i class="lg-blue"></i>Ziyaret <b>'+Math.max(0,d.total-sampleCount)+'</b></span><span><i class="lg-purple"></i>Numune <b>'+sampleCount+'</b></span></div></section><section><h3>4) Plan Durumu</h3>'+donutR(d.uygun,d.disi,d.total)+'</section></div>'
      +'<div class="wr-table"><h3>HAFTALIK ZİYARET LİSTESİ</h3><div><table><thead><tr><th>Firma</th><th>Teknisyen</th><th>Ziyaret Tarihi</th><th>Durum</th><th>Periyot</th><th>Plan Durumu</th></tr></thead><tbody>'+rows+'</tbody></table></div></div>'
      +'<div class="wr-bottom">'+kpiR('Toplam Kayıt',d.total,'visit','blue')+kpiR('Ziyaret Edilen Firma',d.unique,'company','navy')+kpiR('Numune',sampleCount,'sample','purple')+kpiR('Plan Dışı',d.disi,'warn','red')+'</div></div>';
  }
